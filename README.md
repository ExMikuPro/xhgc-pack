...
3. 安装 luavm：
   - 确保 luavm 可执行文件位于 `tool/bin/luavm`
   - 字节码编译指令格式为 `tool/bin/luavm --compile input.lua output.luac`
...
1. **ICON 段**：处理图标文件，转换为 ARGB8888 语义、BGRA 字节序的 raw 数据
...
```json
{"step": "entry", "status": "error", "message": "luavm not found"}
```
